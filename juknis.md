...
1. [BAB I: PENDAHULUAN](#bab-i-pendahuluan)
   ...
   - 1.3 Kebutuhan Sistem & Akses Browser
...
3. [BAB III: PETUNJUK OPERASIONAL ADMINISTRATOR (ADMIN)](#bab-iii-petunjuk-operasional-administrator-admin)
   ...
   - 3.3 Manajemen Master Data Kamar
...
4. [BAB IV: PETUNJUK OPERASIONAL RESEPSIONIS](#bab-iv-petunjuk-operasional-resepsionis)
   ...
   - 4.2 Prosedur Layanan Check-in Peserta
   - 4.3 Pendaftaran Peserta Baru di Lokasi (*On-the-spot Registration*)
   - 4.4 Prosedur Layanan Check-out Peserta
   - 4.5 Monitoring Kamar Kosong & Penghuni Aktif
5. [BAB V: PETUNJUK MONITORING & ANALITIK PIMPINAN](#bab-v-petunjuk-monitoring--analitik-pimpinan)
   ...
   - 5.3 Laporan Utilisasi Per Gedung
...
### 1.1 Gambaran Umum Sistem
**Aplikasi Sistem Manajemen Asrama PPSDMAP** adalah platform berbasis web yang dirancang khusus untuk mempermudah, mendigitalkan, dan mengotomatisasi seluruh alur operasional pengelolaan fasilitas asrama di lingkungan PPSDM Aparatur Perhubungan.

Sistem ini mengintegrasikan seluruh tahapan mulai dari:
1. Pengelolaan master data fasilitas (gedung, kamar, kapasitas).
2. Pendataan agenda program diklat dan kepesertaan (baik input satuan maupun import Excel).
3. Layanan kedatangan (*check-in*) dengan pencarian otomatis peserta dan penempatan kamar siap huni.
4. Layanan kepulangan (*check-out*) serta pelepasan status kamar secara *real-time*.
...
| 2 | **Resepsionis** | Melayani operasional harian meja depan (*front office*), proses check-in tamu, check-out tamu, monitoring kamar kosong, dan monitoring penghuni aktif. | Dashboard, Check-in, Check-out, Kamar Kosong, Penghuni Aktif, Tambah Peserta. |
...
### 2.1 Halaman Login
1. Buka browser dan masukkan alamat URL aplikasi asrama (contoh: `http://localhost:8000` atau alamat IP/Domain server yang ditentukan).
...
- **Kamar Kosong & Terisi**: Status ketersediaan kamar secara *real-time*.
- **Total Peserta & Diklat**: Data akumulasi kegiatan pelatihan dan peserta.
- **Penghuni Aktif**: Jumlah peserta yang sedang menempati kamar saat ini.
...
Menu ini digunakan untuk mendata seluruh blok/gedung asrama (contoh: Gedung A, Gedung B, Asrama Garuda, dsb).
...
### 3.3 Manajemen Master Data Kamar (`/admin/kamar`)
Menu ini digunakan untuk mendata unit-unit kamar yang berada di dalam masing-masing gedung.

#### A. Menambah Kamar Baru:
...
5. **Kapasitas**: Masukkan daya tampung kamar (contoh: *1 orang*, *2 orang*, dsb).
6. **Status Awal**: Tentukan status awal (*Kosong* atau *Dalam Perawatan*).
7. Klik **Simpan Kamar**.

#### B. Filter & Pencarian Kamar:
- Gunakan dropdown filter gedung dan filter status kamar untuk menyaring daftar kamar secara cepat.
...
- **Tab Laporan Per Gedung**: Rekapitulasi kapasitas dan tingkat hunian per gedung.
- **Tab Laporan Per Diklat**: Distribusi kamar per program diklat.
...
3. **Tabel Riwayat Transaksi Terbaru**: Menampilkan 5 transaksi terakhir yang tercatat di sistem.

---

### 4.2 Prosedur Layanan Check-in Peserta (`/resepsionis/checkin`)
Prosedur ini dilakukan saat peserta diklat tiba di meja resepsionis untuk mengambil kunci kamar.

```
ALUR CHECK-IN:
[Peserta Tiba] ➔ [Cari Nama Peserta (Live Combobox)] ➔ [Pilih Kamar Kosong] ➔ [Simpan & Check-in] ➔ [Status Kamar Menjadi 'Terisi']
```
...
3. **Pilih Kamar Kosong**:
   - Pilih unit kamar dari daftar dropdown kamar kosong. Daftar ini dikelompokkan rapi per gedung *(hanya kamar yang berstatus kosong/siap huni yang muncul)*.
4. **Tanggal & Waktu Masuk (Check-in)**:
   - Waktu otomatis terisi saat ini. Anda dapat mengubahnya jika diperlukan.
5. **Konfirmasi & Simpan**:
   - Klik tombol **Simpan & Check-in**.
6. **Hasil Otomatis Sistem**:
   - Sistem mencatat transaksi menginap.
   - Status kamar yang dipilih otomatis berubah menjadi **Terisi**.
   - Nama peserta otomatis masuk ke daftar **Penghuni Aktif**.
   - Notifikasi sukses akan muncul di layar.
...
### 4.4 Prosedur Layanan Check-out Peserta (`/resepsionis/checkout`)
Prosedur ini dilakukan saat agenda diklat selesai dan peserta mengembalikan kunci kamar sebelum meninggalkan asrama.

```
ALUR CHECK-OUT:
[Peserta Kembalikan Kunci] ➔ [Buka Menu Check-out] ➔ [Cari Nama Peserta / Kamar] ➔ [Klik 'Proses Check-out'] ➔ [Kamar Otomatis 'Kosong']
```
...
6. **Hasil Otomatis Sistem**:
   - Status transaksi peserta berubah menjadi **Selesai**.
   - Tanggal & jam keluar tercatat permanen di riwayat laporan.
   - Status kamar yang ditinggalkan otomatis kembali menjadi **Kosong** dan langsung tersedia untuk tamu berikutnya.

---

### 4.5 Monitoring Kamar Kosong & Penghuni Aktif

#### A. Menu Kamar Kosong (`/resepsionis/kamar-kosong`):
- Menampilkan seluruh kamar yang saat ini kosong dan siap dialokasikan.
- Resepsionis dapat memfilter daftar berdasarkan gedung tertentu.
- Dilengkapi tombol cepat **"Check-in ke Kamar Ini"** pada setiap kartu kamar.

#### B. Menu Penghuni Aktif (`/resepsionis/penghuni-aktif`):
- Menampilkan daftar lengkap seluruh peserta yang sedang aktif tidur/menginap di asrama hari ini.
- Memuat informasi: Nama, Instansi, Diklat, Gedung, Nomor Kamar, dan Waktu Masuk.
- Dilengkapi filter pencarian berdasarkan gedung maupun program diklat.
...
Halaman ini menyajikan ringkasan visual performa asrama:
- **Indikator Tingkat Okupansi (*Occupancy Rate*)**: Persentase kapasitas asrama yang sedang digunakan.
...
- **Aktivitas Hunian Terbaru**: Daftar tamu yang baru saja melakukan check-in.
...
### 5.3 Laporan Utilisasi Per Gedung (`/pimpinan/laporan-gedung`)
Menampilkan analisis beban dan keterisian per unit gedung:
1. Tabel menyajikan: Nama Gedung, Total Kamar, Kamar Terisi, Kamar Kosong, Total Kapasitas Orang, dan Persentase Okupansi.
2. Klik tombol **Lihat Kamar** pada gedung yang diinginkan.
3. Di bagian bawah akan muncul rincian denah status seluruh kamar di gedung tersebut (hijau untuk kamar kosong, merah untuk kamar terisi lengkap dengan nama penghuni).
...
### Q1: Mengapa nama peserta tidak muncul pada kolom dropdown saat check-in?
> **Jawaban:** 
> Ada 2 kemungkinan:
> 1. Peserta tersebut sudah melakukan check-in dan saat ini tercatat sedang aktif menginap di kamar lain.
...
### Q2: Mengapa kamar yang ingin dipilih tidak muncul di daftar kamar formulir check-in?
> **Jawaban:**
> Formulir check-in hanya menampilkan kamar yang berstatus **Kosong**. Jika kamar tidak muncul, artinya kamar tersebut sedang berstatus **Terisi** atau belum didaftarkan oleh Administrator di menu Master Kamar.

---

### Q3: Bagaimana jika ada peserta yang berpindah kamar?
> **Jawaban:**
...
### Q4: Format file apa yang didukung untuk Import Data Peserta?
> **Jawaban:**
> Format file yang didukung adalah Microsoft Excel (`.xlsx`, `.xls`) atau Comma Separated Values (`.csv`). Pastikan kolom mengikuti template resmi yang dapat diunduh pada halaman Import.

---

### Q5: Bagaimana jika saya lupa kata sandi akun?
> **Jawaban:**
...
### Q6: Bagaimana jika hasil filter tanggal pada laporan tidak memuat data?
> **Jawaban:**

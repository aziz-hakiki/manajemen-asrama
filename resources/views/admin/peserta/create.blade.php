<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2 text-sm text-slate-500">
            ...
            <span>/</span>
            <span class="font-semibold text-slate-800">Tambah Peserta</span>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl">
        <div class="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 sm:p-8">
            <div class="mb-6">
                <h2 class="text-lg font-bold text-slate-800">Tambah Peserta Baru</h2>
                <p class="text-xs text-slate-500 mt-1">Masukkan data lengkap peserta dan kaitkan dengan kegiatan diklat.</p>
            </div>

            <form action="{{ route('admin.peserta.store') }}" method="POST" class="space-y-5">
                @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        {{-- Alert Real-time AJAX saat NIP/NIK sudah terdaftar --}}
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function checkNip(nip) {
                ...
                fetch(`{{ route('admin.peserta.check-nip') }}?nip_nik=${encodeURIComponent(nip)}`)
                    .then(res => res.json())
                    .then(data => {
            ...
            // Jalankan pengecekan jika ada nilai lama saat reload
            if (nipInput.value.trim().length > 0) {
                checkNip(nipInput.value.trim());
            }
        });
    </script>
</x-app-layout>
